...
def set_logger(log_path, log_name='seqrec', mode='a'):
    """Set up log file. Avoid adding duplicate handlers."""
    logger = logging.getLogger(log_name)

    # 关键修复：如果已经初始化过，直接返回，避免重复添加 handler
    if logger.handlers:
        return logger
    logger.setLevel(logging.INFO)
    logger.propagate = False  # 阻止传播（你已做，保留）
    # 文件处理器
    fh = logging.FileHandler(log_path, mode=mode)
    fh.setLevel(logging.INFO)
    # 控制台处理器（可选：如果你不需要每条日志都打到控制台，可以去掉）
    ...
    logger.addHandler(ch)  # 如果你只想写文件，删掉这一行
    return logger
...
def parse_args():
    ...
    parser.add_argument("--data_name", "--data", "--d",
    ...
    parser.add_argument("--batch_size", default=256,
                        type=int, help="number of batch_size")
    ...
    if args.model_name.lower() == 'flow':
        parser.add_argument("--predictor_blocks", default=4, type=int)
        parser.add_argument("--predictor_dropout_prob",
                            default=0.2, type=float)
        parser.add_argument("--w_cfm", default=1.0, type=float)
        parser.add_argument("--w_ce", default=1.0, type=float)
        parser.add_argument("--n_steps", default=5, type=int)

    if args.model_name.lower() == 'bert4rec':
        parser.add_argument("--mask_ratio", default=0.2, type=float)

    if args.model_name.lower() == 'caser':
    ...
    if args.model_name.lower() == 'gru4rec':
        parser.add_argument("--gru_hidden_size", default=64,
                            type=int, help="hidden size of GRU")

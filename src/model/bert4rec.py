...
from model.abstract import SequentialRecommender
...
class BERT4RecModel(SequentialRecommender):
    def __init__(self, args):
        ...
        self.item_embeddings = nn.Embedding(
            args.item_size+1, args.hidden_size, padding_idx=0)
    ...
    def calculate_loss(self, input_ids, answers, neg_answers, same_target, user_ids):
        ...
        masked_index = torch.zeros(input_ids.size(
            0), mask_num).to(input_ids.device)

        for i in range(input_ids.size(0)):
            masked_index[i, :] = torch.multinomial(torch.ones(
                input_ids.size(1)), mask_num, replacement=False)
        ...
        pred_index_map = self.multi_hot_embed(
            masked_index, input_ids.size(-1))  # [B*mask_len max_len]
        # [B mask_len] -> [B mask_len max_len] multi hot
        pred_index_map = pred_index_map.view(masked_index.size(
            0), masked_index.size(1), -1)  # [B mask_len max_len]
        ...
        # [item_num H]
        test_item_emb = self.item_embeddings.weight[: self.n_items]
        logits = torch.matmul(
    ...
    def reconstruct_test_data(self, item_seq):
        ...
        padding = self.mask_token * \
            torch.ones(item_seq.size(0), dtype=torch.long,
                       device=item_seq.device)  # [B]
        item_seq = torch.cat(
            (item_seq, padding.unsqueeze(-1)), dim=-1)  # [B max_len+1]
        item_seq = item_seq[:, 1:]
